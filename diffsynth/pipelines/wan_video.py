...
from ..models.wan_video_rmod import RInterMod
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.float16, tokenizer_path=None):
        ...
        self.rintermod = RInterMod(20, 4096, 2)
    ...
    def predict_latents(self, y, prompt_emb, cond_idx, num_frames=33):
        actual_frames = int(1 + (num_frames - 1) / 4)
        cond_idx = 0 if (cond_idx == 0) else (actual_frames - 1)
        base_latent = y[:,:,0,:,:]
        _, _, hh, ww = base_latent.shape
        base_latent = rearrange(base_latent, 'b c h w -> b (h w) c')
        if cond_idx == (actual_frames - 1):
            prompt_emb = \
                [
                    torch.mean(
                        prompt_emb[item:item+4], dim=0
                    ).unsqueeze(0)
                    for item in range(0, num_frames-1, 4)
                ] \
                + [prompt_emb[-1].unsqueeze(0)]
            
        else:
            prompt_emb = \
                [prompt_emb[0].unsqueeze(0)] + \
                [
                    torch.mean(
                        prompt_emb[item:item+4], dim=0
                    ).unsqueeze(0)
                    for item in range(1, num_frames, 4)
                ]

        new_y = []
        for idx in range(len(prompt_emb)):
            if idx == cond_idx:
                new_y.append(base_latent)
            else:
                new_y.append(self.rintermod(base_latent, prompt_emb[idx]))
        new_y = torch.cat(new_y, dim=0)
        new_y = rearrange(new_y.unsqueeze(0), 'b f (h w) c -> b c f h w', h=hh, w=ww)
        prompt_emb = torch.cat(prompt_emb, dim=0)
        return new_y, prompt_emb
    ...
    def __call__(
        ...
        if ablation_type == 'narrator-error':
            if random.random() < 0.5:
                o_ind = list(range(num_frames))
                random.shuffle(o_ind)
                for i in o_ind[:3]:
                    prompt[i] = 'Generate an frame that is consistent within the scene.'
        # Parameter check
        ...
        if use_diff_reasoner:
            if "y" in image_emb:
                image_emb["y"], prompt_emb_posi["context"] = self.predict_latents(image_emb["y"], prompt_emb_posi["context"], cond_index, num_frames)
            prompt_emb_posi["context"] = rearrange(prompt_emb_posi["context"], 'b c d -> (b c) d').unsqueeze(0)
        # Motion Controller
